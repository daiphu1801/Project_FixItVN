package com.fixit.ui.worker.orders.invoice;
...
public class WorkerInvoiceFragment extends BaseFragment<FragmentWorkerInvoiceBinding> {
    ...
    private void setupInvoiceItems() {
        ...
        List<WorkerOrdersViewModel.ExtraCostItem> extras = viewModel.extraItems.getValue();
        long extraTotal = 0;
        if (extras != null) {
            for (WorkerOrdersViewModel.ExtraCostItem item : extras) {
