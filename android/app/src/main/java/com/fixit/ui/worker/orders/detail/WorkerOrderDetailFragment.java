package com.fixit.ui.worker.orders.detail;
...
public class WorkerOrderDetailFragment extends BaseFragment<FragmentWorkerOrderDetailBinding> {
    ...
    protected void setupViews() {
        ...
        binding.btnCompleteOrderDetail.setOnClickListener(v -> {
            WorkerOrdersViewModel.JobStatus current = viewModel.currentStatus.getValue();
            if (current == WorkerOrdersViewModel.JobStatus.REPAIRING) {
    ...
    protected void observeData() {
        ...
        if (orderId != null) {
            com.fixit.data.model.WorkerOrder order = viewModel.getOrderById(orderId);
            if (order != null) {
                bindOrderData(order);
    ...
    private void bindOrderData(com.fixit.data.model.WorkerOrder order) {
    ...
    private void updateTimelineUI(WorkerOrdersViewModel.JobStatus status) {
        ...
        if (status == WorkerOrdersViewModel.JobStatus.COMPLETED) {
