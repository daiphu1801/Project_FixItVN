...
public class WorkerOrderDetailFragment extends BaseFragment<FragmentWorkerOrderDetailBinding> {
    ...
    protected void setupViews() {
        ...
        // Khởi tạo MapView và Thiết lập click listener cho bản đồ
        if (binding.mapView != null) {
            binding.mapView.onCreate(savedInstanceState);
        }
        binding.cardMapPlaceholder.setOnClickListener(v -> {
            if (currentOrder != null && currentOrder.getAddress() != null) {
                String address = currentOrder.getAddress();
                try {
                    android.net.Uri mapUri = android.net.Uri.parse("geo:0,0?q=" + android.net.Uri.encode(address));
                    android.content.Intent mapIntent = new android.content.Intent(android.content.Intent.ACTION_VIEW, mapUri);
    ...
    private void bindOrderData(WorkerOrder order) {
        ...
        // Tải bản đồ theo địa chỉ
        loadMap(order.getAddress());
    ...
    private void loadMap(String address) {
        if (address == null || address.trim().isEmpty() || binding.mapView == null) return;

        binding.mapView.getMapAsync(googleMap -> {
            // Chuyển địa chỉ sang LatLng dùng Geocoder
            new Thread(() -> {
                try {
                    ...
                    android.util.Log.e("WorkerOrderDetail", "Geocoding failed: " + e.getMessage());
                }
            }).start();
        });
    }
    ...
    private void updateTimelineUI(JobStatus status) {
        ...
        if (status == JobStatus.REPAIRING) {
            ...
            if (currentOrder != null && ("CASH".equalsIgnoreCase(currentOrder.getPaymentMethod())
                ...
                paymentBinding.tvPaymentStatus
                        .setBackgroundTintList(ColorStateList.valueOf(Color.parseColor("#eff6ff")));
